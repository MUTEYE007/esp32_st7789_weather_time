...
void uiTask(void *pvParameters) {
  ...
  while (1) {
    ...
    // OTA overlay goes BEFORE everything else — no clock/warning/ui leaks through
    if (otaPhase >= 0) {
      ...
      if (otaPhase != lastPh) {
        ...
        if (otaPhase == 1) {
          ...
          fillArea(20, 60, 0, 14, COLOR_GREEN);
      ...
      // Download progress — only update bar + %, no full clear
