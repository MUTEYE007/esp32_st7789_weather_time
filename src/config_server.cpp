...
function sc(el,ok,t,ms){el.className='st '+(ok?'ok':'er');el.style.display='block';el.textContent=t;var d=ms!==undefined?ms:ok?3000:8000;if(d>0)setTimeout(()=>el.style.display='none',d)}
...
function checkUpdate(){var st=document.getElementById('chkSt');var btn=document.getElementById('chkBtn');btn.disabled=true;btn.textContent='检查中...';sc(st,1,'正在检查...');fetch('/check-update').then(r=>r.text()).then(t=>{var ok=!t.startsWith('ERR:');var persist=ok&&(t.includes('正在下载')||t.includes('正在检查'));sc(st,ok,ok?t:t.substring(4),persist?60000:undefined);btn.disabled=false;btn.textContent='检查更新'}).catch(e=>{sc(st,0,'连接超时');btn.disabled=false;btn.textContent='检查更新'})}
